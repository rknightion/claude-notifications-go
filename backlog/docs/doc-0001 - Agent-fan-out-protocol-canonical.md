...
> `m7kni/agent-docs` at commit `711db6c`. This copy is authoritative for `claude-notifications-go`, so an agent
...
**Routing is deliberately not restated here.** A Claude Code session already loads the model-routing and subagent-dispatch policies from its global rules directory on every request, before anything asks for this document. Those two own the routing test, the model tiers and the dispatch mechanics, and **they win
