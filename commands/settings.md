...
Based on your answers, I'll create `claude-notifications-go/config.json` under the active Claude configuration root (`$CLAUDE_CONFIG_DIR`, then `$CLAUDE_HOME`, otherwise `~/.claude`):
...
1. First, get the stable config directory and create it:
   ```bash
   mkdir -p "${CLAUDE_CONFIG_DIR:-${CLAUDE_HOME:-$HOME/.claude}}/claude-notifications-go"
   echo "${CLAUDE_CONFIG_DIR:-${CLAUDE_HOME:-$HOME/.claude}}/claude-notifications-go/config.json"
   ```
...
3. Also copy to legacy path for backward compat with older binary versions:
   ```bash
   cp "${CLAUDE_CONFIG_DIR:-${CLAUDE_HOME:-$HOME/.claude}}/claude-notifications-go/config.json" "${PLUGIN_ROOT}/config/config.json" 2>/dev/null || true
   ```
...
Configuration file: <active Claude config root>/claude-notifications-go/config.json
```
...
- Or manually edit `claude-notifications-go/config.json` under the active Claude configuration root

**Webhook Configuration:**
If you enabled webhooks, you'll need to manually edit that profile-local `config.json` to add:
