...
1. Use `just dev-local-install` first when you are changing install/update behavior.
2. Use `just e2e-smoke` when you need a real `claude` process to execute hooks.
3. Use `just dev-real-local` only when you must validate behavior inside your real Claude environment.
...
```bash
just dev-local-update
just dev-local-reset
```
...
- Run `just e2e-status` to see what the current machine supports before running a mode.
...
```bash
just build
binary="$(find bin -maxdepth 1 -type f -name 'claude-notifications-*' -print -quit)"
"$binary" version
```
...
```powershell
just build
$binary = Get-ChildItem -Path "bin/claude-notifications-*.exe" | Select-Object -First 1 -ExpandProperty FullName
& $binary version
```
...
```bash
echo '{"session_id":"local-debug","tool_name":"ExitPlanMode"}' | "$binary" handle-hook PreToolUse
```

Windows PowerShell:

```powershell
'{"session_id":"win-debug","tool_name":"ExitPlanMode"}' | & $binary handle-hook PreToolUse
```
...
```bash
just build
binary="$(find bin -maxdepth 1 -type f -name 'claude-notifications-*.exe' -print -quit)"
echo '{"session_id":"win-debug","tool_name":"ExitPlanMode"}' | "$binary" handle-hook PreToolUse
```
...
```bash
REAL_CLAUDE_HOME="$HOME/.claude-dev/claude-notifications-go" just e2e-status
```
...
```bash
just dev-real-local
just dev-real-status
just dev-real-remote
```

There is also a convenience toggle:

```bash
just dev-real-toggle
```
...
- Prefer `just dev-local-install` before touching your real Claude environment.
- Prefer `just e2e-smoke` when you need real hook execution but do not need to mutate the installed marketplace.
- Use `just dev-real-local` only for end-to-end checks that depend on the real installed plugin path.
- Keep `just build` up to date before testing install/init flows so the plugin can reuse the local `bin/` binary.
...
1. `just dev-local-install`
2. `just dev-local-bootstrap`
3. `just dev-local-status`

For changes to hooks/notifier/runtime behavior:

1. `just e2e-smoke`
2. `just e2e-smoke-installed`
...
- Claude process debug log: printed by the `just e2e-*` recipes for each run
