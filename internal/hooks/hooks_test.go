...
func TestHandler_CodexDocumentedPayloadStatuses(t *testing.T) {
	tests := []struct {
		...
		{name: "question", event: "PermissionRequest", payload: `{"session_id":"codex-question","turn_id":"turn-2","cwd":"/workspace","hook_event_name":"PermissionRequest","permission_mode":"default","tool_name":"Bash","tool_input":{"command":"just build","description":"Run the build outside the sandbox"}}`, wantStatus: analyzer.StatusQuestion},
